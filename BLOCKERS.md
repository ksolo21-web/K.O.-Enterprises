...
- OANDA practice data: both owner-named secrets reached run 34081410690. It failed locally at an overly fixed account-ID format check before broker access. The parser now accepts variable numeric component widths and trims surrounding whitespace while preserving injection defenses. A new manual run on main after merge is required to test the repaired revision; the connector cannot dispatch a new run. The underlying secret value and connection remain unverified.
