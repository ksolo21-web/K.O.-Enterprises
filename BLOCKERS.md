...
- OANDA practice data: Kaleb renewed the report that demo keys are present on 2026-09-07. Secret names and values cannot be verified through this GitHub connection, no OANDA variables are present locally, and the inspected repository run list contains 20 CI runs and no OANDA run. The manual workflow expects `OANDA_DEMO_TOKEN` and `OANDA_DEMO_ACCOUNT_ID`. The connection can read results but exposes no dispatch action; the owner can run it in GitHub Actions. No broker reads or orders occurred here.
