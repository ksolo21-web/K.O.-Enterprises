"""Read-only, sealed-data fetch for OANDA Practice Wave 3.

Wave 3 deliberately moves away from the opened H1 USD-pair universe. It uses
H4 candles for six cross-currency pairs and exports only the first 80 percent
of completed candles for strategy research. The final 20 percent never leaves
the ephemeral GitHub runner until a candidate is fixed in source.

No order, trade, position, transfer, withdrawal, live-hostname, or scheduling
...
class Wave3PracticeClient:
    """Strict fixed-route GET-only client for Wave 3 market research."""
    ...
    @classmethod
    def from_environment(cls) -> "Wave3PracticeClient":
        return cls(
    ...
    def _get(self, resource: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if resource not in allowed:
            raise LabError("Unapproved OANDA Wave 3 route.")
        ...
        try:
            with self._opener.open(request, timeout=25) as response:
                ...
                if len(body) > MAX_RESPONSE:
                    raise LabError("OANDA Wave 3 response exceeded the size limit.")
        except HTTPError as error:
            raise LabError(f"OANDA Wave 3 request returned HTTP {error.code}.") from None
        except (URLError, TimeoutError):
            raise LabError("OANDA Wave 3 request failed.") from None
        try:
            payload = json.loads(body)
        except json.JSONDecodeError:
            raise LabError("OANDA Wave 3 returned invalid JSON.") from None
        if not isinstance(payload, dict):
            raise LabError("OANDA Wave 3 returned an unexpected response.")
        return payload

    def summary(self) -> dict[str, Any]:
        account = self._get("summary").get("account")
        if not isinstance(account, dict) or account.get("currency") != "USD":
            raise LabError("Wave 3 requires a USD-denominated Practice account.")
        return {
    ...
    def available_instruments(self) -> set[str]:
        ...
        if not isinstance(rows, list):
            raise LabError("OANDA returned invalid Wave 3 instrument metadata.")
        return {
            row["name"]
            for row in rows
    ...
    def candles(self, instrument: str, count: int = CANDLE_COUNT) -> dict[str, Any]:
        if instrument not in INSTRUMENTS:
            raise LabError("Unapproved Wave 3 instrument.")
        if type(count) is not int or not 1200 <= count <= CANDLE_COUNT:
            raise LabError("Wave 3 candle count must be between 1200 and 5000.")
        payload = self._get(
...
def _price(row: dict[str, Any], side: str, key: str) -> float:
    try:
        value = _finite(row[side][key], "Wave 3 candle price")
    except (KeyError, TypeError):
        raise LabError("Malformed Wave 3 candle price.") from None
    if value <= 0:
        raise LabError("Wave 3 candle prices must be positive.")
    return value
...
def parse_candles(payload: object, instrument: str) -> tuple[Bar, ...]:
    ...
    if not isinstance(rows, list) or not 1200 <= len(rows) <= CANDLE_COUNT:
        raise LabError("Expected 1200-5000 Wave 3 candle records.")
    bars: list[Bar] = []
    for row in rows:
        if not isinstance(row, dict):
            raise LabError("Malformed Wave 3 candle record.")
        if row.get("complete") is not True:
            continue
        try:
            stamp = datetime.fromisoformat(str(row["time"]).replace("Z", "+00:00"))
        except (KeyError, ValueError):
            raise LabError("Malformed Wave 3 candle timestamp.") from None
        if stamp.tzinfo is None:
            raise LabError("Wave 3 candle timestamps require a timezone.")
        ...
        if not (
            ...
            raise LabError("Invalid Wave 3 OHLC or crossed bid/ask values.")
        if bars and stamp <= bars[-1].time:
            raise LabError("Wave 3 candles must be strictly chronological and unique.")
        bars.append(bar)
    if len(bars) < 1150:
        raise LabError("Too few completed Wave 3 candles.")
    return tuple(bars)


def fetch_full_bundle(client: Wave3PracticeClient, count: int = CANDLE_COUNT) -> dict[str, Any]:
    summary = client.summary()
    if summary["open_trade_count"] or summary["pending_order_count"]:
        raise LabError(
            "Existing Practice trades or pending orders found; Wave 3 research will not alter them."
        )
    available = client.available_instruments()
    missing = [instrument for instrument in INSTRUMENTS if instrument not in available]
    if missing:
        raise LabError("One or more Wave 3 cross-currency instruments are unavailable.")
...
def export_preholdout(bundle: object) -> dict[str, Any]:
    if not isinstance(bundle, dict) or bundle.get("status") != "wave3_full_ephemeral_market_data":
        raise LabError("Invalid Wave 3 full-data bundle.")
    raw = bundle.get("candles")
    if not isinstance(raw, dict) or set(raw) != set(INSTRUMENTS):
        raise LabError("Wave 3 bundle must contain the complete cross-pair whitelist.")
    ...
    for instrument in INSTRUMENTS:
        ...
        if split < 1000 or split >= len(bars):
            raise LabError("Invalid Wave 3 sealed split.")
        rows = payload.get("candles")
        if not isinstance(rows, list):
            raise LabError("Malformed Wave 3 candle payload.")
    ...
    return {
        "status": "wave3_preholdout_market_data",
        "environment": "practice",
        "granularity": GRANULARITY,
...
def _read_json(path: str) -> Any:
    ...
    if source.stat().st_size > MAX_RESPONSE * len(INSTRUMENTS):
        raise LabError("Wave 3 input file is too large.")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Fetch and seal OANDA Practice Wave 3 cross-pair data.")
    commands = parser.add_subparsers(dest="command", required=True)
    fetch = commands.add_parser("fetch")
    fetch.add_argument("--output", default="state/oanda-wave3-full.json")
    ...
    export.add_argument("--output", default="state/oanda-wave3-preholdout.json")
    args = parser.parse_args(argv)
    try:
        if args.command == "fetch":
            payload = fetch_full_bundle(Wave3PracticeClient.from_environment(), args.count)
            _write_json(args.output, payload)
            print(
                f"Captured {len(INSTRUMENTS)} H4 cross pairs for sealed Wave 3 research. No orders sent."
            )
        else:
            payload = export_preholdout(_read_json(args.input))
            _write_json(args.output, payload)
            print(
                "Exported only the first 80% of Wave 3 completed candles; final 20% remains sealed."
            )
        return 0
    except (LabError, OSError, json.JSONDecodeError) as error:
        print(str(error) if isinstance(error, LabError) else "Wave 3 file operation failed.", file=sys.stderr)
        return 2
