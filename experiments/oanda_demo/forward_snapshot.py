"""Sanitized candle-only snapshot for forward Practice experiment design."""
...
def build_snapshot(client: ForwardPracticeClient) -> dict[str, Any]:
    ...
    return {
        ...
        "current_pricing_included": False,
        "markets": rows,
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Create a sanitized OANDA Practice candle snapshot."
    )
    ...
    try:
        ...
        print(
            "Forward candle-snapshot leaders: "
            + ", ".join(
                f"{instrument} score24={row['score_24']:.3f} "
                f"z={row['zscore_24']:.3f} efficiency={row['efficiency_24']:.3f}"
        ...
        print(
            ...
            else "Forward snapshot file operation failed.",
            file=sys.stderr,
        )
        return 2
